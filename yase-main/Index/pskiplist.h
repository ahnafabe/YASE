...
struct PSkipListNode {
  ...
  PSkipListNode(uint32_t nlevels, uint32_t ksize, uint32_t vsize) : 
...
struct PSkipList {
  ...
  PSkipList(std::string name, uint32_t key_size, uint32_t payload_size);
  ...
  // @key: pointer to the key
  // @payload: data entry 
  RID NewNode(uint32_t levels, const char *key, const char *payload);
